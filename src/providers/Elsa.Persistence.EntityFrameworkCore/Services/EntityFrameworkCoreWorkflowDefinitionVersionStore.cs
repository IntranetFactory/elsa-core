    ...
    public class EntityFrameworkCoreWorkflowDefinitionVersionStore : IWorkflowDefinitionVersionStore
    {
        ...
        public async Task<WorkflowDefinitionVersion> SaveAsync(WorkflowDefinitionVersion definitionVersion, CancellationToken cancellationToken = default)
        {
            var existingEntity =
                await dbContext
                    ...
                    .FirstOrDefaultAsync(x => x.TenantId == definitionVersion.TenantId && x.VersionId == definitionVersion.Id, cancellationToken);
        ...
        public async Task<WorkflowDefinitionVersion> GetByIdAsync(int? tenantId, string id, CancellationToken cancellationToken = default)
        {
            var query = dbContext
                ...
                .Include(x => x.Activities)
                .Include(x => x.Connections)
                .Where(x => x.TenantId == tenantId && x.VersionId == id);
        ...
        public async Task<int> DeleteAsync(int? tenantId, string id, CancellationToken cancellationToken = default)
        {
            ...
            var definitionVersionRecords = await dbContext.WorkflowDefinitionVersions
                .Where(x => x.TenantId == tenantId && x.DefinitionId == id)
                .ToListAsync(cancellationToken);
